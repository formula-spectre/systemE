...
(setq exec-path '("/usr/local/bin" "/bin" "/usr/bin"))
...
;;             "busybox" "mount" "-o" "nosuid,noexec,nodev" "-t" "proc" "proc" "/proc")))

;; (progn
;;   (message "Mounting sys...")
;;   (message "%s"
;;            (process-exit-code-and-output
;;             "busybox" "mount" "-o" "nosuid,noexec,nodev" "-t" "sysfs" "sys" "/sys")))

;; (progn
;;   (message "Mounting run...")
;;   (message "%s"
;;            (process-exit-code-and-output
;;             "busybox" "mount" "-o" "mode=0755,nosuid,nodev" "-t" "tmpfs" "run" "/run")))

;; (progn
;;   (message "Mounting dev...")
;;   (message "%s"
;;            (process-exit-code-and-output
;;             "busybox" "mount" "-o" "mode=0755,nosuid" "-t" "devtmpfs" "dev" "/dev")))
...
;;   (message "%s"
;;            (process-exit-code-and-output
;;             "busybox" "mount" "-o" "mode=0620,gid=5,nosuid" "-nt" "devpts" "devpts" "/dev/pts")))

;; (progn
;;   (message "Mounting shm...")
;;   (message "%s"
;;            (process-exit-code-and-output
;;             "busybox" "mount" "-o" "mode=0777,nosuid,nodev" "-nt" "tmpfs" "shm" "/dev/shm")))
...
(progn
  call-process-shell-command "/usr/bin/s6-init")
